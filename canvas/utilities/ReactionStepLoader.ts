...
class ReactionStepLoader {
  public static loadReactionStepFromPlainObject(
    reactionStepPlainObject: any
  ): ReactionStep {
      ...
      for (const savedMolecule of reactionStepPlainObject["molecules"]) {
        ...
        for (const savedAtom of savedMolecule.atoms) {
          ...
          if (savedAtom.lonePair != null) {
            const lonePair = new LonePair(
              newAtom,
              savedAtom.lonePair.uuid,
              savedAtom.lonePair.angle
            );
        ...
        for (const savedBond of savedMolecule.bonds) {
          const atomOne = Utilities.getAtomWithinMoleculeByID(
            newMolecule,
            savedBond["atomOne"]
          );
          const atomTwo = Utilities.getAtomWithinMoleculeByID(
            newMolecule,
            savedBond["atomTwo"]
          );
    ...
    const savedStraightArrow = JSON.parse(
      reactionStepPlainObject["straightArrow"]
    );
    ...
    if (savedStraightArrow != null) {
      ...
      const restoredStraightArrow = new StraightArrow(straightArrowStartVector);
