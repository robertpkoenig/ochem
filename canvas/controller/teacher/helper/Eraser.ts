...
class Eraser {
  ...
  eraseIonIfClicked() {
    const ion = this.editorController.hoverDetector.ionCurrentlyHovered;
  ...
  eraseArrowIfClicked() {
    ...
    if (arrowHovered != null) {
      ...
      this.editorController.reaction.currentStep.curlyArrows =
        this.editorController.reaction.currentStep.curlyArrows.filter(
          (curlyArrow) => curlyArrow !== arrowHovered
        );
      ReactionSaver.saveReaction(this.editorController.reaction);
    }
  }
