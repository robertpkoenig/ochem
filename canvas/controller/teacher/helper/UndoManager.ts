...
class UndoManager {
  ...
  undo() {
    ...
    if (previousModelJSON) {
      this.addRedoPoint();
      const previousModel: Reaction =
        ReactionLoader.loadReactionFromJSON(previousModelJSON);
  ...
  redo() {
    ...
    if (reactionStateToRestore) {
      ...
      const previousModel: Reaction = ReactionLoader.loadReactionFromJSON(
        reactionStateToRestore
      );
      this.editorController.reaction.replaceWithNewModel(previousModel);
      ReactionSaver.saveReaction(previousModel);
    }
