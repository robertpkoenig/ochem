...
class AtomMover {
  ...
  startDraggingBodyIfPressed() {
    this.mouseStartPosition = {
      x: this.p5.mouseX / this.reaction.zoom,
      y: this.p5.mouseY / this.reaction.zoom,
    };
    this.setAtomsBeingDragged();
  }

  getMoleculeThatAtomIsAPartOf(atom: Atom) {
    return this.reaction.currentStep.molecules.find((molecule) =>
      molecule.atoms.includes(atom)
    );
  }
  ...
  addAtomToMap(atom: Atom) {
    const startPosition = {
      x: atom.getPosVector().x,
      y: atom.getPosVector().y,
    };
  ...
  dragAllAtomsRequired() {
    if (this.atomsBeingDraggedAndTheirStartPositions.size > 0) {
      ...
      this.atomsBeingDraggedAndTheirStartPositions.forEach(
        (originalPosition, atom) =>
