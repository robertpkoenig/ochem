...
class View {
  ...
  constructor(
    ...
    if (userType == UserType.STUDENT) {
      this.studentView = new StudentView(
        p5,
        controller as StudentController,
        this.curlyArrowViewer
      );
    }
  ...
  renderDraftCurlyArrow() {
    const draftArrow = this.controller.curlyArrowCreator.draftArrow;
  ...
  renderStraightArrow() {
    const straightArrow = this.reaction.currentStep.straightArrow;
    if (straightArrow) {
      this.p5.push();
      this.p5.stroke(0);
