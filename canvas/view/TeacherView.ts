...
class TeacherView {
  ...
  private decorateIonIfHovered() {
    ...
    if (hoveredIon != null) {
      if (this.teacherController.pageState.eraserOn) {
        this.drawIonOutline(hoveredIon, RED_OUTLINE_COLOR);
        this.showEraserTip();
      } else if (this.onlyMovingHandOrCurlyArrowActive()) {
  ...
  private decorateLonePairIfHovered() {
    const hoveredLonePair =
      this.teacherController.hoverDetector.lonePairCurrentlyHovered;
  ...
  private onlyMovingHandOrCurlyArrowActive() {
    return (
      ...
      (this.teacherController.pageState.bondType == null &&
        ...
        this.teacherController.pageState.selectedIonType == null &&
  ...
  private drawLonePairOutline(lonePair: LonePair, color: string) {
    ...
    const curylArrowSelected =
      this.teacherController.pageState.arrowType === ArrowType.DOUBLE;
    const { x, y } = lonePair.getPosVector();
    this.p5.push();
    this.p5.fill(color);
  ...
  private showEraserTip() {
    ...
    const boundingRect =
      this.teacherController.panelController.canvasParent.getBoundingClientRect();
  ...
  private decorateBondIfHovered() {
    const hoveredBond =
      this.teacherController.hoverDetector.bondCurrentlyHovered;
  ...
  private renderDraftBond() {
    if (this.teacherController.bondCreator.startAtom != null) {
      ...
      if (this.teacherController.pageState.bondType == BondType.SINGLE) {
        this.p5.stroke(0);
        this.p5.line(
          this.teacherController.bondCreator.startAtom.circle.pos.x,
  ...
  private decorateArrowIfHovered() {
    const arrow =
      this.teacherController.hoverDetector.curlyArrowCurrentlyHovered;
  ...
  private drawArrowOutline(arrow: CurlyArrow, color: string) {
    ...
    this.p5.strokeWeight(ARROW_STROKE_WEIGHT + OUTLINE_THICKNESS * 2);
  ...
  private onlyMovingHandOrArrowIsActive() {}
}
