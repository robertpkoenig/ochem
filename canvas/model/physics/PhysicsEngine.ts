...
class PhysicsEngine {
  ...
  repulseTwoObjects(atomOne: Atom, atomTwo: Atom) {
    ...
    const xForce =
      (distanceVector.x / magnitudeSquared) *
      ATOM_REPULSION_FACTOR *
      massProduct;
    const yForce =
      (distanceVector.y / magnitudeSquared) *
      ATOM_REPULSION_FACTOR *
      massProduct;
  ...
  makeBondLengthCorrect() {
    for (const bond of this.reaction.currentStep.getAllBonds()) {
      ...
      const difBetweenDesiredAndCurrentLength =
        desiredBondLength - currentBondLength;

      const lengthDifAsPercentOfCurrentLength =
        difBetweenDesiredAndCurrentLength / currentBondLength;

      const distanceVectorScaledToDifference = distanceVector.scale(
        lengthDifAsPercentOfCurrentLength / 4
      );
