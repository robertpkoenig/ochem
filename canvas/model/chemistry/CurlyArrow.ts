...
class CurlyArrow {
  ...
  setEndPoints(p5: p5, zoom: number) {
    ...
    if (this.endObject != null) {
      ...
      this.endVector = new Vector(p5.mouseX / zoom, p5.mouseY / zoom);
    }
  }
  ...
  setTrianglePoints(p5: p5) {
    ...
    const trianglePointOneX =
      p5.cos(a + angleScale + angleAdjust) * triangleLength + this.endVector.x;
    const trianglePointOneY =
      p5.sin(a + angleScale + angleAdjust) * triangleLength + this.endVector.y;

    this.trianglePointOne = new Vector(trianglePointOneX, trianglePointOneY);

    const trianglePointTwoX =
      p5.cos(a - angleScale + angleAdjust) * triangleLength + this.endVector.x;
    const trianglePointTwoY =
      p5.sin(a - angleScale + angleAdjust) * triangleLength + this.endVector.y;
