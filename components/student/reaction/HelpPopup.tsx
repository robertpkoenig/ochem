...
const HelpPopup = (props: IProps) => {
  const hintText = (
    <div className="bg-gray-100 rounded-md py-2 px-3">
      <h3 className="text-gray-800 font-semibold">Hint From The Lecturer</h3>
      <p className="whitespace-pre-line">{props.reaciton.hint}</p>
    </div>
  );

  const generalInstructions = (
    <div className="bg-gray-100 rounded-md py-2 px-3">
      <h3 className="text-gray-800 font-semibold">General Instructions</h3>
      <p>
  ...
  return (
    <PopupBackground popupCloseFunction={props.togglePopup}>
      <div className="flex flex-col bg-white shadow rounded-md py-3 px-4">
        <div className="pb-2 text-lg font-semibold">Help</div>
        <div className="w-[700px] flex flex-col gap-2 rounded-sm">
          <Show if={props.reaciton.hint.length > 0}>{hintText}</Show>
          {generalInstructions}
        </div>
      </div>
    </PopupBackground>
