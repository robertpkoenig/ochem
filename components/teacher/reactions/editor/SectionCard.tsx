...
export default function SectionCard(props: IProps) {
  ...
  function createReaction(reactionName: string) {
    ...
    const sectionRecordDocLocation = doc(db, MODULES, props.module.uuid);
  ...
  const reactionListEmptyState = (
    <EmptyState text="This section has no reactions yet" />
  );
    ...
    <div className=" px-6 py-4 flex flex-col gap-4 bg-gray-100 overflow-hidden rounded-md">
      <div className="flex flex-row justify-between items-center">
        ...
        <div className="flex flex-row content-center gap-1">
          <RoundButton icon={ChevronDownIcon} onClick={incrementSectionOrder} />

          <RoundButton icon={ChevronUpIcon} onClick={decrementSectionOrder} />

          <RoundButton icon={XMarkIcon} onClick={toggleSectionDeletePopup} />
        </div>
      </div>

      <div>{reactionList ? reactionList : reactionListEmptyState}</div>
