...
function ListOfSteps(props: IProps) {
  ...
  function createNewStep() {
    ...
    const newStep =
      ReactionStepLoader.loadReactionStepFromPlainObject(lastStepJSON);
  ...
  function deleteStep(
    ...
    props.state.reaction.steps = props.state.reaction.steps.filter((step) => {
  ...
  const listOfStepButtons = props.state.reaction?.steps.map((step) => (
    <li
      onMouseDown={() => setCurrentStep(step.uuid)}
      key={step.uuid}
      className={classNames(
        "group relative px-2 flex gap-1 items-center rounded-md  hover:bg-indigo-700 hover:text-white cursor-pointer",
        step === props.state.reaction.currentStep && "text-white bg-indigo-700"
      )}
    >
      <span className="text-sm font-medium my-1">Step {step.order + 1}</span>
      <button
        onMouseDown={(e) => deleteStep(e, step.uuid)}
        className={classNames(
          "invisible absolute top-0 right-0 text-indigo-300 bg-indigo-500 hover:text-white hover:bg-indigo-400 rounded-full p-0.5",
    ...
    <div className="flex flex-row gap-4 items-center">
      <nav className="flex flex-row items-center">
        <ol className="rounded-md flex gap-2 text-indigo-400 ">
          <Show if={props.state.reaction != null}>{listOfStepButtons}</Show>
        </ol>
      </nav>
      <button
