...
class DeleteManager {
    ...
    public static deleteBond(reaction: Reaction, bondToDelete: Bond) {
        ...
        if (!this.doListsHaveCommonItem(listOne, listTwo)) {
            ...
            const molOne = Utilities.findMoleculeContainingAtom(reaction, bondToDelete.atoms[0])
        ...
        while (!q.isEmpty()) {
            ...
            for (const bond of nextAtom.bonds) {
                ...
                for (const atom of bond.atoms) {
                    ...
                    // if this body has not been seen, add it to the queue
                    // and the list of connected bodies
                    if (atom != nextAtom &&
                        !connectedAtoms.includes(atom)) {
    ...
    private static doListsHaveCommonItem(listOne: Atom[], listTwo: Atom[]): boolean {
    ...
    private static assignListOfAtomsToNewMolecule(atoms: Atom[], reaction: Reaction) {
    ...
    private static removeListOfAtomsFromMolecule(atoms: Atom[], molecule: Molecule) {
        molecule.atoms = molecule.atoms.filter((atom: Atom) => !atoms.includes(atom))
    }
    ...
    private static resassignBondsToCorrectMolecules(reaction: Reaction): void {
        ...
        for (const bond of allBonds) {
            ...
            const moleculeContainingAtom =
                Utilities.findMoleculeContainingAtom(reaction, atomOne)
