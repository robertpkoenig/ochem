...
function StudentReactionPage(props: IProps) {
  ...
  const [state, setState] = useState<IStudentState>(() => ({
    reaction: null,
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, REACTIONS, props.reactionId)).then((doc) => {
  ...
  const reactionComplete =
    state.reaction?.currentStep ==
    state.reaction?.steps[state.reaction?.steps.length - 1];
          ...
          <main className="">
            <div className="w-1200 h-700 mx-auto pb-12 flex flex-row gap-5">
              <Show if={introPopupVis}>
                <HelpPopup
                  reaciton={state.reaction}
                  togglePopup={toggleIntroPopup}
                />
              </Show>
              {/* p5 canvas */}
              <div
                id={CANVAS_PARENT_NAME}
                className="bg-white h-700 rounded-lg shadow flex-grow relative"
              >
                <Show if={!reactionComplete}>{progressBar}</Show>
