...
export default function Analytics() {
  ...
  const [moduleAnalyticsRecord, setModuleAnalyticsRecord] =
    useState<ModuleAnalyticsRecord>(null);
  const [numUniquesLastSevenDays, setNumUniquesLastSevenDays] =
    useState<number>(0);
  ...
  async function fetchAndProcessData() {
    ...
    const datesFormattedForDisplay = createListOfDatesFormattedForDisplay();
    const numberOfStudentsByDayLastSevenDays = tallyStudentNumbers(
  ...
  async function getFirebaseDateRecords(): Promise<DateRecord[]> {
    ...
    const dateRecordsCollectionLocation = collection(
      db,
      MODULE_ANALYTICS_RECORDS,
      moduleId,
      DATE_RECORDS
  ...
  function createListOfDatesForLastSevenDaysFormattedLikeFirebaseDateRecords(): string[] {
    ...
    let date = new Date();
    for (let i = 0; i < 7; i++) {
      datesForLastSevenDaysFormattedLikeFirebase.push(
  ...
  return (
    <ScreenWithLoading loading={loading}>
      ...
      >
        ...
        <p className="mb-2 font-medium text-gray-500">Unique Users</p>
