...
export default function Modules() {
  ...
  const [createModulePopupVisible, setCreateModulePopupVisible] =
    useState<boolean>(false);
  const [deleteModulePopupVisible, setDeleteModulePopupVisible] =
    useState<boolean>(false);
  ...
  function createModule(name: string, subtitle: string) {
    ...
    setDoc(
      doc(db, MODULE_ANALYTICS_RECORDS, moduleId),
      newModuleAnalyticsRecord
    );
  }

  // Creates the abridged module document
  function createModuleListing(
    name: string,
    moduleId: string,
    creationDate: string
  ) {
  ...
  const moduleListEmptyState = (
    <EmptyState text="You don't have any modules yet" />
  );
  ...
  const moduleList: ReactNode = (
    <div className=" overflow-hidden rounded-md">
      <ul className="space-y-4">
        {moduleListings
          .sort(function (a: ModuleListing, b: ModuleListing) {
            return parseInt(a.creationDate) - parseInt(b.creationDate);
          })
          .map((moduleListing: ModuleListing) => (
            <li
              key={moduleListing.uuid}
              className="px-6 py-4 bg-gray-100 rounded-md"
            >
              <ModuleCard
    ...
    <ScreenWithLoading loading={loading}>
      ...
      >
        <div className="flex flex-col gap-2">
          {moduleListings.length > 0 ? moduleList : moduleListEmptyState}
        </div>
